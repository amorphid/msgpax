defmodule Msgpax.Ext do
  ...
  may want, however, is to unpack that extension back to a `RepByte` struct. We
  can do this by passing an "extensions" module to unpacking functions such as
  `Msgpax.unpack/2` through the `:ext` option. This extensions module has to
  implement the `Msgpax.Ext.Unpacker` behaviour and will be used by Msgpax to
  unpack extensions to arbitrary terms.

  For our `RepByte` example, we could create an unpacker module:
